  ...
  return (
    <header
      className={`fixed w-full z-50 transition-all duration-300 ${
        scrolled ? 'bg-slate-900/90 shadow-lg shadow-orange-900/10 backdrop-blur-md border-b border-white/5' : 'bg-transparent'
      }`}
    >
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex justify-between items-center h-20">
          ...
          >
            <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg flex items-center justify-center transform group-hover:rotate-180 transition-transform duration-500 shadow-lg shadow-orange-600/20 border border-white/10">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {NAV.map(({ path, label }) => (
              <button
                ...
                className={`text-sm font-medium uppercase tracking-wider hover:text-orange-400 transition-colors relative group ${
                  activePath === path ? 'text-orange-500' : 'text-slate-300'
                }`}
              >
                {label}
                <span
          ...
          <div className="hidden md:flex items-center gap-4">
            <a
              href={phoneHref}
              className="flex items-center gap-2 bg-orange-500 hover:bg-orange-400 text-white px-5 py-2.5 rounded font-bold transition-all transform hover:scale-105 shadow-lg shadow-orange-500/30 ring-2 ring-orange-500/50 hover:ring-orange-400"
            >
              <Phone size={18} />
              <span>{phone || 'Ara'}</span>
            </a>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-slate-900/95 backdrop-blur-xl absolute w-full border-t border-slate-800 shadow-2xl min-h-[50vh] z-50">
          ...
          <div className="flex flex-col p-4 gap-4">
            ...
            <a
              href={phoneHref}
              className="bg-gradient-to-r from-orange-500 to-orange-400 text-white w-full py-4 rounded-xl font-bold flex justify-center items-center gap-2 mt-4 shadow-lg shadow-orange-500/20 active:scale-95 transition-transform"
            >
